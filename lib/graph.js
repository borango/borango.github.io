...
,
l : { text : "[l]ist of selected nodes (use [,] and [ ] to put on clipboard)",		f : function()
    {
...
,
n : { text : "travel [n]orth (forward) direction upon next CLICK", 	f : function()
    {
      NEXT_CLICK_DIRECTION = DIRECTION_NORTH
    }}
,
o : { text : "keep [o]utersection (clear intersection)",		f : function()
    {
      var intersection = document.querySelectorAll( "g[tag2]" )
      if( intersection.length > 0 )
      {
...
,
r : { text : "[r]emove a single node upon next CLICK", 			f : function()
    {
    }}
,
s : { text : "travel [s]outh (backward) direction upon next CLICK",	f : function()
    {
      NEXT_CLICK_DIRECTION = DIRECTION_SOUTH
    }}
,
u : { text : "copy [u]rl of focussed node to clipboard",		f : function()
    {
...
y : { text : "[y]ank (copy) focussed node ID to clipboard",		f : function(document)
    {
      navigator.clipboard.writeText( focussed.id ).then( function() { }, function() { } );
    }}
,
',':{ text : "copy selection to clipboard, separated by [,]", f : function() { copy_selection_to_clipboard( ',' ) } }
,
' ':{ text : "copy selection to clipboard, separated by [ ]", f : function() { copy_selection_to_clipboard( ' ' ) } }


,
'?':{ text : "Help",							f : function()
      {
...
function copy_selection_to_clipboard( separator )
{
 let result = ""
